...
/**
 *
 * Classe Criada para o Controlar mostrar o JogoModoSinglePLayer
 */
public class JogoSinglePlayer extends Application {
    /*
    Default Constructor
    ...
    */ 
    public JogoSinglePlayer(int level, Integer Pontuacao, boolean EE){
    ...
    private Stage theStage; //Seta Janela 
    ...
    public void start(Stage Stage_ignorar) throws Exception {
        ...
        TransicaoDeFase ProxFase = new TransicaoDeFase(Level+1,Pontuacao,1,EE);
        ...
        Canhao c = new Canhao(ImgCanhao,200,670,80,57); //Cria Canhao do Jogador
        
        
        Engine engine = new Engine(c, Level,Pontuacao); //Cria Engine para modoDeJogo
        ...
        public void handle(long currentNanoTime)
            {
                ...
                {    
                    ...
                    if(input.contains("A")) //Move o canhao para esquerda, A do teclado
                        if(c.getX()>4) //Caso esteja viavel ir para a esquerda
                            c.MudaPosicaoX(-6);
                    if(input.contains("D")) //Move o canhao para direita, D do teclado
                        if(c.getX()<1267) //Caso esteja viavel ir para a direita
                            c.MudaPosicaoX(6);
                    if(input.contains("SPACE")) //Ativa Tiro, Espaco do teclado
                    {    
                        c.Atira(); //canhao atira
                        input.remove("SPACE");
                    }     
